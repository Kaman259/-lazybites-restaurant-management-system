...
def get_firebase_app() -> firebase_admin.App:
    """Initialise and return the default Firebase Admin application."""
    ...
    if not credentials_path.exists():
        raise AppException(
            message=(
                ...
                "Add firebase-service-account.json inside backend."
            ),
            status_code=500,
        )
    ...
    try:
        ...
        raise AppException(
            message="Firebase Admin could not be initialised.",
            status_code=500,
        ) from exception


def verify_firebase_id_token(id_token: str) -> dict[str, Any]:
    """Verify a Firebase client ID token and return its claims."""
    ...
    if not firebase_uid:
        raise AppException(
            message="Firebase user identifier is missing.",
            status_code=401,
        )

    if not email:
        raise AppException(
            message="An email address is required for this account.",
            status_code=401,
        )
